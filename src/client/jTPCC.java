...
public class jTPCC implements jTPCCConfig
{
    ...
    private DBCollector dbCollector = null;
    ...
    log.info("Term-00,  (c) 2020, Nicolas Martin");
    log.info("Term-00, +-------------------------------------------------------------+");
    log.info("Term-00, ");
    ...
    String  iSSL                = ini.getProperty("SSL","False");
    log.info("Term-00, "+iSSL);
    ...
    String    dbCollectorScript   = getProp(ini, "dbCollectorScript");
    ...
    if (databaseDriverLoaded && resultDirectory != null)
    {
        ...
        //dbCollector
        if (dbCollectorScript != null)
        {
            dbCollector = new DBCollector(dbCollectorScript,
                runID,
                Integer.parseInt(getProp(ini, "dbCollectorInterval")),
                iConn,
                iSSL,
                iUser,
                iPassword,
    ...
    if(databaseDriverLoaded)
    {
        ...
        switch (dbType)
        {
            case DB_POSTGRES:
            /*
            * ADD SSL cnx to PG database
            */
            dbProps.setProperty("ssl", iSSL);
    ...
    if(terminalsStarted == 0)
    {
        ...
        // Stop the DBCollector, if it is active.
        if (dbCollector != null)
        {
            dbCollector.stop();
        dbCollector = null;
